// =============================================================
//  El Atlas N°4 — Comparación migratoria entre países (chart 15)
// =============================================================
//
...
function mcm_layoutCountryLabels(sortedData, barWidth, plotArea, labelCodes) {
  ...
  const gapX = fontSize * MCM_LABEL_GAP_K;
  ...
  const maxRow = Math.max(0, Math.min(MCM_BEND_ROW_COUNT - 1,
    Math.floor((yLine - 4 - (plotArea.bottom + MCM_BEND_ROW_OFFSET)) / MCM_BEND_ROW_GAP)));
  let iRun = 0;
  while (iRun < nLbl) {
    ...
    for (let k = iRun; k <= jRun; k++) {
      ...
      filaQuiebre[k] = plotArea.bottom + MCM_BEND_ROW_OFFSET + Math.min(r, maxRow) * MCM_BEND_ROW_GAP;
    }
    iRun = jRun + 1;
  }
...
function setupMigrantesCompDownloadCSV() {
  document.querySelectorAll('button.download[data-chart="15-csv"]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      a.href = url;
      a.download = lang === 'en'
        ? 'the-atlas-04-immigrant-comparison.csv'
        : 'el-atlas-04-inmigrante-comparacion.csv';
