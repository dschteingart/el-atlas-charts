// =============================================================
//  El Atlas N°4 — Chart 6: Latinobarómetro (barras 2024 + líneas históricas)
// =============================================================
...
function setupLatinoCSV() {
  document.querySelectorAll('button.download[data-chart="6-csv"]').forEach(btn => btn.addEventListener('click', () => {
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8' })); a.download = lang === 'en' ? 'the-atlas-04-latinobarometro.csv' : 'el-atlas-04-latinobarometro.csv'; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(a.href);
  }));
}
