// El Atlas N°4 — WRP (discriminacion vivida), vista MAPA. CLON VERBATIM de
// vdem-mapa.js con ids renumerados (state 22->25, chart25->chart25, c25->c25);
// los datos los sirve wrp-adapter.js. NO editar la logica aca.
...
function vm_waves() { return vd_yearList(state[25].cat); }
...
function vm_ramp() {
  return vd_peorEsMas(state[25].cat) ? VM_COLORS : VM_COLORS.slice().reverse();
}
...
function vm_updateSubtitle() {
  const el = document.querySelector('.chart-subtitle[data-i18n="c25-subtitle-tpl"]');
  if (!el) return;
  ...
  const catA = vd_varLabelOf(state[25].cat);
  const tpl = (typeof t === 'function') ? t('c25-subtitle-tpl') : '';
  if (tpl) el.textContent = tpl.replace('{CAT}', catA).replace('{PERIODO}', vm_waveLabel(state[25].wave));
}

function drawVdemMapa() {
  const svg = document.getElementById('chart25');
  ...
  const cat = state[25].cat, wave = state[25].wave;
  ...
  vm_features().forEach(f => {
    ...
    const datoAhora = () => vm_dataFor(state[25].cat, state[25].wave)[iso];
    path.addEventListener('mouseenter', (e) => { vm_setHover(d); vm_showTooltip(e, iso, datoAhora(), state[25].cat); });
    path.addEventListener('mousemove', (e) => vm_posTooltip(e));
    path.addEventListener('mouseleave', () => { vm_setHover(''); vm_hideTooltip(); });
    path.addEventListener('click', (e) => { vm_setHover(d); vm_showTooltip(e, iso, datoAhora(), state[25].cat); });
    g.appendChild(path);
  });
  ...
  if (typeof atlasSetHeading === 'function') atlasSetHeading('25', false, { title: 'c25-title', titleNeutral: 'c25-title-neutral' });
}
...
function vm_drawLegend(svg, breaks) {
  ...
  nt.textContent = (typeof t === 'function') ? t('c25-legend-nodata') : 'Sin dato'; g.appendChild(nt);
}
...
function vm_showTooltip(e, iso, v, cat) {
  const tt = document.getElementById('tooltip25'); if (!tt) return;
  const L = (typeof t === 'function') ? t : (k) => k;
  if (!v) {
    tt.innerHTML = `<strong>${vm_name(iso)}</strong><div class="tt-row tt-muted">${L('c25-tt-nodata')}</div>`;
  } else {
    const traj = vm_trajectory(iso, cat);
    tt.innerHTML = `<strong>${vm_name(iso)}</strong>`
      + `<div class="tt-row"><span>${L('c25-tt-value')}</span><span>${vd_fmtVal(v.pct, vd_dec())}</span></div>`
      + `<div class="tt-row tt-row-sub"><span>${L('c25-tt-year')}</span><span>${v.year}</span></div>`
      + (traj.length >= 2 ? `<div class="tt-sub" style="margin-top:4px;">${L('c25-tt-trend')}</div>` + vm_sparkline(traj, v.year) : '');
  }
  tt.style.display = 'block'; tt.style.opacity = '1';
  vm_posTooltip(e);
}
function vm_posTooltip(e) {
  const tt = document.getElementById('tooltip25'); if (!tt || !tt.parentElement) return;
...
function vm_hideTooltip() { const tt = document.getElementById('tooltip25'); if (tt) tt.style.opacity = '0'; }
...
function setupMapaCat() {
  ...
  sel.addEventListener('change', () => {
    ...
    vm_stopPlay();
    state[25].cat = sel.value;
    // El anio elegido puede caer fuera del rango de la variable nueva.
    const ys = vd_years(state[25].cat);
    if (ys.length) {
      state[25].wave = Math.min(Math.max(state[25].wave, ys[0]), ys[ys.length - 1]);
      ...
      if (input) {
        ...
        input.value = Math.max(0, ys.indexOf(state[25].wave));
      }
      if (disp) disp.textContent = vm_waveLabel(state[25].wave);
    }
    drawVdemMapa();
  });
}
...
function setupMapaWave() {
  ...
  if (!input || vd_yearList(state[25].cat).length < 2) {
    const grp = document.getElementById('vm-wave-group'); if (grp) grp.style.display = 'none';
    return;
  }
  const waves = vd_yearList(state[25].cat);
  input.min = 0; input.max = waves.length - 1; input.step = 1;
  const idxOf = (w) => Math.max(0, waves.findIndex(x => x.w === w));
  const sync = () => { input.value = idxOf(state[25].wave); if (disp) disp.textContent = vm_waveLabel(state[25].wave); };
  input.addEventListener('input', () => {
    vm_stopPlay();
    state[25].wave = waves[+input.value].w;
    if (disp) disp.textContent = vm_waveLabel(state[25].wave);
    drawVdemMapa();
  });
  if (playBtn) playBtn.addEventListener('click', () => {
    ...
    state[25].wave = waves[0].w; sync(); drawVdemMapa();
...
function setupMapaCSV() {
  document.querySelectorAll('button.download[data-chart="25-csv"]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const lang = (typeof LANG !== 'undefined') ? LANG : 'es';
      const cat = state[25].cat, year = state[25].wave;
      let csv = '';
      ...
      a.download = lang === 'en' ? 'the-atlas-04-skin-colour-discrimination-map.csv' : 'el-atlas-04-discriminacion-color-piel-mapa.csv';
...
function initVdemMapa() {
  ...
  if (!state[25]) state[25] = { cat: VM_DEFAULT_CAT, wave: lastWave };
  if (state[25].wave == null) state[25].wave = lastWave;
  const sel = document.getElementById('vm-cat-select'); if (sel) sel.value = state[25].cat;
