...
(function setupPNGExport() {
  ...
  const IS_N4 = typeof ES_N4_INTOLERANCIA !== 'undefined';
  // N°1 (bienestar/violencia): sentinel en su i18n-issue.js (Fase 3b, rewire a
  // lib/ — antes tenía una copia local de este archivo).
  const IS_N1 = typeof ES_N1_BIENESTAR !== 'undefined';
  const IS_N3 = !IS_ESPECIAL && !IS_N4 && !IS_N1
  ...
  const FILENAMES = IS_N1 ? {
    ...
    '1': { es: 'el-atlas-04-ranking-vecinos.png',   en: 'the-atlas-04-neighbours-ranking.png' },
    '2': { es: 'el-atlas-04-evolucion-vecinos.png', en: 'the-atlas-04-neighbours-trend.png'   },
    '3': { es: 'el-atlas-04-mapa-vecinos.png',      en: 'the-atlas-04-neighbours-map.png'     },
    '4': { es: 'el-atlas-04-perfil-pais.png',       en: 'the-atlas-04-country-profile.png'    },
    '5': { es: 'el-atlas-04-declarado-implicito.png', en: 'the-atlas-04-declared-implicit.png' },
    '6': { es: 'el-atlas-04-latinobarometro.png',   en: 'the-atlas-04-latinobarometro.png'    },
    '7': { es: 'el-atlas-04-prioridad-nativos.png', en: 'the-atlas-04-jobs-priority.png'      },
    '8': { es: 'el-atlas-04-barrio.png',            en: 'the-atlas-04-neighbourhood.png'      },
    '9': { es: 'el-atlas-04-perfil-inmigrante.png', en: 'the-atlas-04-immigrant-profile.png'  },
    '11': { es: 'el-atlas-04-grupo-discriminado.png', en: 'the-atlas-04-discriminated-group.png' },
    '12': { es: 'el-atlas-04-mas-discriminado.png', en: 'the-atlas-04-most-discriminated.png' },
    '13': { es: 'el-atlas-04-barrio-comparacion.png', en: 'the-atlas-04-neighbourhood-comparison.png' },
    '14': { es: 'el-atlas-04-prioridad-comparacion.png', en: 'the-atlas-04-jobs-priority-comparison.png' },
    '15': { es: 'el-atlas-04-inmigrante-comparacion.png', en: 'the-atlas-04-immigrant-comparison.png' },
    '16': { es: 'el-atlas-04-discriminado-comparacion.png', en: 'the-atlas-04-discriminated-comparison.png' },
    '17': { es: 'el-atlas-04-barrio-evolucion.png', en: 'the-atlas-04-neighbourhood-trend.png' },
    '18': { es: 'el-atlas-04-intolerancia-pib.png', en: 'the-atlas-04-intolerance-gdp.png' },
    '19': { es: 'el-atlas-04-correlaciones.png', en: 'the-atlas-04-correlations.png' },
    '20': { es: 'el-atlas-04-exclusion-pib.png', en: 'the-atlas-04-exclusion-gdp.png' },
    '21': { es: 'el-atlas-04-exclusion-ranking.png', en: 'the-atlas-04-exclusion-ranking.png' },
    '22': { es: 'el-atlas-04-exclusion-mapa.png', en: 'the-atlas-04-exclusion-map.png' },
    '24': { es: 'el-atlas-04-discriminacion-color-piel.png', en: 'the-atlas-04-skin-colour-discrimination.png' },
    '25': { es: 'el-atlas-04-discriminacion-color-piel-mapa.png', en: 'the-atlas-04-skin-colour-discrimination-map.png' },
    '23': { es: 'el-atlas-04-exclusion-evolucion.png', en: 'the-atlas-04-exclusion-trend.png' },
    '26': { es: 'el-atlas-04-tabla-perfil.png',        en: 'the-atlas-04-profile-table.png' }
  } : IS_ESPECIAL ? {
    ...
    '5': { es: 'el-atlas-03-elo-trayectorias.png',   en: 'the-atlas-03-elo-trajectories.png'   },
    // 6-12 agregados 2026-08-10: hasta acá caían al fallback genérico
    // (el-atlas-03-chart-7.png), mientras que los 10 del ex especial sí tenían
    // nombre descriptivo. Ahora los 22 de "El Atlas del fútbol" están parejos.
    '6': { es: 'el-atlas-03-natividad.png',          en: 'the-atlas-03-foreign-born.png'       },
    '7': { es: 'el-atlas-03-ligas.png',              en: 'the-atlas-03-club-leagues.png'       },
    '8': { es: 'el-atlas-03-mapa-nacimientos.png',   en: 'the-atlas-03-map-birthplaces.png'    },
    '9': { es: 'el-atlas-03-origenes.png',           en: 'the-atlas-03-player-origins.png'     },
    '10': { es: 'el-atlas-03-altura.png',            en: 'the-atlas-03-height.png'             },
    '11': { es: 'el-atlas-03-tecnicos.png',          en: 'the-atlas-03-managers.png'           },
    '12': { es: 'el-atlas-03-edad.png',              en: 'the-atlas-03-age.png'                }
  } : {
    '1': {
  ...
  // extra del PNG.
  const SHOWS_LEGEND = chartId => IS_N1
    ? (chartId === '1' || chartId === '2')   // scatters del N°1; el 3 (líneas) tiene end-labels inline
    : IS_N4
    ? false   // N°4: el ranking pinta sus barras de un solo color y no lleva leyenda
    : IS_ESPECIAL
    ? (chartId === '4')
    : IS_N3
      ? (chartId === '1' || chartId === '4')
  ...
  async function preloadCanvasFonts(fontShorthands) {
    ...
    // Pequeño delay para asegurar que el canvas font-cache se actualizó: dos
    // frames. OJO con el race contra el timeout — no es paranoia. En una
    // pestaña oculta (otra solapa al frente, ventana minimizada) el browser
    // NO corre requestAnimationFrame, así que el await se colgaba para
    // siempre y el export nunca terminaba: ni PNG, ni error, ni nada. Con la
    // pestaña a la vista los dos frames llegan en ~32 ms, mucho antes de los
    // 300, así que el resultado no cambia para nadie.
    await Promise.race([
  ...
  function legendItems() {
    // N°1: las 10 regiones propias de lib/regions.js (REGION_ORDER/COLORS),
    // con nombres traducidos vía t('reg.<region>') de su i18n-issue.js.
    if (IS_N1) {
  ...
  async function downloadChartPNG(chartId, options) {
    ...
    let   titleText    = block.querySelector('.chart-title')?.textContent.trim()    || '';
    let   subtitleText = block.querySelector('.chart-subtitle')?.textContent.trim() || '';
    // REGLA: lo que el lector escribió en el panel del editor GANA sobre los
    // hooks del chart. Los hooks arman variantes según el estado (un título
    // más corto para el PNG, la nota de fuentes del año activo…), y hasta
    // ahora pisaban el texto custom salvo que cada renderer se acordara de
    // ceder. Centralizado acá para los tres textos (título, subtítulo, nota).
    const aePngCfg = (window.AtlasEditor && window.AtlasEditor.getConfig)
      ? window.AtlasEditor.getConfig() : null;
    const aePngLang = aePngCfg ? (aePngCfg.lang || (typeof LANG !== 'undefined' ? LANG : 'es')) : 'es';
    const aePngTx = (aePngCfg && aePngCfg.texts && aePngCfg.texts[aePngLang]) || {};
    const custom = (k) => ((aePngTx[k] || '') + '').trim();
    // Hook opcional: el chart puede dar un título distinto para el PNG. En la
    // página el título puede permitirse una coletilla que ubica ("…en el
    // mundo") porque no compite con nada; en el PNG cada renglón de 64 px se
    // lo saca al gráfico. Sin hook no cambia nada.
    if (!custom('title') && typeof window.onBeforePngExportGetTitle === 'function') {
      try {
        const ov = window.onBeforePngExportGetTitle(chartId);
        if (ov) titleText = ov;
      } catch (_) {}
    }
    ...
    if (!custom('subtitle') && typeof window.onBeforePngExportGetSubtitle === 'function') {
      try {
    ...
    // el modo raw/adj activo). atlasApplyEditorTexts ya escribió el caption
    // custom en el pie, así que sourceText lo trae: el hook cede ante él.
    if (!custom('caption') && typeof window.onBeforePngExportGetSourceText === 'function') {
      try {
    ...
    // Hook opcional: el chart puede pedir otra caja para la nota — más ancha
    // (ratio) y/o más chica (escala sobre sourceSize/sourceLineH). Lo pide un
    // gráfico cuyo alto se lo comen las filas, no el pie: ahí una nota de 8
    // renglones angostos le roba tamaño al gráfico. Sin hook no cambia nada,
    // así que los PNG ya publicados conservan su layout exacto.
    let srcRatio = SOURCE_MAX_RATIO, srcSize = sourceSize, srcLineH = sourceLineH;
    let firmaExacta = false;
    if (typeof window.onBeforePngExportGetSourceLayout === 'function') {
      try {
        const lay = window.onBeforePngExportGetSourceLayout(chartId, format);
        if (lay) {
          if (lay.ratio  > 0) srcRatio = lay.ratio;
          if (lay.escala > 0) {
            srcSize  = Math.round(sourceSize  * lay.escala);
            srcLineH = Math.round(sourceLineH * lay.escala);
          }
          firmaExacta = true;
        }
      } catch (_) {}
    ...
    let attribW = attribText ? mctx.measureText(attribText).width : 0;
    // OJO: en mobile-first la firma NO se dibuja en una línea sino en dos
    // ("El Atlas" arriba, "Daniel Schteingart" abajo — ver más abajo), así que
    // su huella real es el más ancho de esos dos renglones. Medir la cadena
    // entera de un saque da 399 px en el cuadrado contra los 214 que ocupa de
    // verdad: 185 px que la nota perdía contra un fantasma. Se corrige sólo
    // donde el chart lo pide, para no mover los PNG ya publicados.
    if (attribText && firmaExacta && mobileFirst) {
      const partes = attribText.split('·').map(s => s.trim()).filter(Boolean);
      if (partes.length >= 2) {
        mctx.font = `700 ${attribSize}px "Source Sans 3", -apple-system, sans-serif`;
        const wFirma1 = mctx.measureText(partes[0]).width;
        mctx.font = `600 ${Math.round(attribSize * 0.78)}px "Source Sans 3", -apple-system, sans-serif`;
        const wFirma2 = mctx.measureText(partes.slice(1).join(' · ')).width;
        attribW = Math.max(wFirma1, wFirma2);
      }
    }
    ...
    const sourceMaxW = attribText
      ? Math.min(innerW * srcRatio, innerW - attribW - attribGap)
      : innerW * srcRatio;

    mctx.font = `400 ${srcSize}px "Source Sans 3", -apple-system, sans-serif`;
    ...
    const lastLineH = Math.max(srcLineH, attribSize * 1.15);
    const sourceH = sourceLines > 0
      ? (sourceLines - 1) * srcLineH + lastLineH
      : 0;
    ...
    // El mapa del N°2 pide alto DINÁMICO (window.__atlasPngDynamicHeight): el
    // lienzo se ajusta al contenido (modo B) aunque haya formato elegido, así el
    // mapa —que cambia de aspecto según el continente (mundo apaisado, América
    // vertical)— nunca queda flotando con medio lienzo vacío arriba/abajo. Se
    // mantiene el ancho (nominalW) y el chrome mobile-first del formato worldmap.
    // El resto de los charts sigue en modo A (alto FIJO = nominalH).
    const dynamicH = !!window.__atlasPngDynamicHeight;
    let svgW, svgH, svgX, H;
    if (format && PNG_FORMATS[format] && !dynamicH) {
    ...
    if (format && PNG_FORMATS[format] && !dynamicH && sourceText) {
    ...
    if (sourceText) {
      ...
      ctx.fillStyle = PALETTE.inkSoft;
      ctx.textBaseline = 'top';
      ctx.font = `400 ${srcSize}px "Source Sans 3", -apple-system, sans-serif`;
      wrapText(ctx, sourceText, padX, y, sourceMaxW, srcLineH);
    }
    ...
    if (!IS_N3 && !IS_ESPECIAL && !IS_N4 && !IS_N1 && chartId === '1' && state && state[1]) {
      const mode = state[1].mode || 'raw';  // 'raw' | 'adj'
      filename = FILENAMES['1']?.[`${lang}_${mode}`] || `el-atlas-02-chart-1-${mode}.png`;
    } else {
      const prefix = IS_N1 ? 'el-atlas-01' : IS_N4 ? 'el-atlas-04' : IS_ESPECIAL ? 'el-atlas-especial' : IS_N3 ? 'el-atlas-03' : 'el-atlas-02';
