...
(function setupPNGExport() {
  ...
  const IS_N3 = !IS_ESPECIAL && !IS_N5 && !IS_N1
  ...
  const SHOWS_LEGEND = chartId => IS_N1
    ...
    : IS_N5
    ? false   // N°5: el ranking pinta sus barras de un solo color y no lleva leyenda
    : IS_ESPECIAL
    ? (chartId === '4')
    : IS_N3
      ? (chartId === '1' || chartId === '4')
  ...
  async function downloadChartPNG(chartId, options) {
    ...
    if (!IS_N3 && !IS_ESPECIAL && !IS_N5 && !IS_N1 && chartId === '1' && state && state[1]) {
      const mode = state[1].mode || 'raw';  // 'raw' | 'adj'
      filename = FILENAMES['1']?.[`${lang}_${mode}`] || `el-atlas-02-chart-1-${mode}.png`;
    } else {
      const prefix = IS_N1 ? 'el-atlas-01' : IS_N5 ? 'el-atlas-04' : IS_ESPECIAL ? 'el-atlas-especial' : IS_N3 ? 'el-atlas-03' : 'el-atlas-02';
