...
(function () {
  ...
  // declararlo con data-editor-sizes="ticks,labels"; este mapa es el default
  // por chart, y si no hay ni una cosa ni la otra se muestran las cuatro.
  const SIZE_KEYS = {
    // N°1
  ...
  function sizeKeysFor(chartId) {
    ...
    if (declaradas.length) return declaradas.filter(k => ALL_SIZE_KEYS.indexOf(k) >= 0);
  ...
  function syncSizeSliders() {
    ...
    ALL_SIZE_KEYS.forEach(k => {
      const fila = ae.panelEl.querySelector(`input[data-ae-size="${k}"]`);
      const row = fila ? fila.closest('.ae-slider-row') : null;
