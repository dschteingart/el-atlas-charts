...
//   - Tamaños: título, subtítulo y nota del PNG, zoom del gráfico, ticks,
//     etiquetas, "elemento especial" y axis-titles (cada chart declara cuáles
//     obedece).
//   - Formato del PNG (público / newsletter / cuadrado / mobile).
//   - Vista previa en vivo del PNG, si la página la ofrece
//     (window.__atlasPngPreview).
...
(function () {
  ...
  // Título, subtítulo y nota viven solo en el PNG: sus defaults son los de la
  // casa en los formatos mobile-first (52 / 32 / 18). "chart" es el zoom del
  // contenido del gráfico, en %. Como en las perillas viejas, estos números
  // son de fábrica: mientras el lector no mueve una perilla, manda el preset
  // del chart (ver atlasEditorSize).
  const DEFAULT_SIZES = {
    ticks: 11,
    labels: 10,
    special: 10,
  ...
  // Rango y nombre de cada perilla, en el orden en que aparecen en el PNG (de
  // arriba abajo). Los tamaños de texto del PNG son grandes (se lee a un
  // tercio en el celu), por eso los topes superan holgado a los presets.
  const SIZE_META = {
    title:     { min: 24, max: 80,  step: 1,   es: 'Título',         en: 'Title' },
    subtitle:  { min: 14, max: 56,  step: 1,   es: 'Subtítulo',      en: 'Subtitle' },
    chart:     { min: 50, max: 160, step: 5,   es: 'Gráfico (zoom)', en: 'Chart (zoom)', unit: '%' },
    ticks:     { min: 6,  max: 40,  step: 0.5, es: 'Ticks ejes',     en: 'Axis ticks' },
    axisTitle: { min: 6,  max: 40,  step: 0.5, es: 'Títulos de eje', en: 'Axis titles' },
    labels:    { min: 6,  max: 40,  step: 0.5, es: 'Etiquetas país', en: 'Country labels' },
    special:   { min: 6,  max: 40,  step: 0.5, es: 'Especial',       en: 'Special' },
    caption:   { min: 10, max: 36,  step: 0.5, es: 'Nota',           en: 'Caption' }
  };
  const SIZE_ORDER = ['title', 'subtitle', 'chart', 'ticks', 'axisTitle', 'labels', 'special', 'caption'];

  const DEFAULT_FORMAT = 'newsletter';
  // El editor arranca en el formato con el que la página exporta sin editor
  // (__atlasDefaultPngFormat: 'square', 'worldmap'…). Con 'newsletter' fijo,
  // abrir ?nl=1 cambiaba el tamaño del PNG respecto del que baja el lector, y
  // las perillas se ajustaban sobre una imagen que nadie iba a publicar.
  function pageDefaultFormat() {
    const f = window.__atlasDefaultPngFormat;
    if (f && typeof PNG_FORMATS !== 'undefined' && PNG_FORMATS[f]) return f;
  ...
  const SPECIAL_LABEL = {
    ...
    // chart-4 del N°2 (vista líneas) y chart-3 del N°1: el slider "especial"
    // controla los nombres al final de cada línea.
    'ranking': { es: 'End-labels', en: 'End-labels' },
    'homicidios-series': { es: 'End-labels', en: 'End-labels' },
    // N°4: los charts de ranking dibujan una tabla de promedios por región.
    'n5-ranking': { es: 'Tabla regional', en: 'Regional table' },
    'n5-barrio-comp': { es: 'Tabla regional', en: 'Regional table' },
    'n5-prioridad-comp': { es: 'Tabla regional', en: 'Regional table' }
  };

  // Nombre de una perilla cuando en ese chart significa otra cosa que el
  // default (en el N°5 las etiquetas no siempre son países).
  const KNOB_LABEL = {
    'evolucion-fama':   { labels:  { es: 'Etiquetas de rubro', en: 'Field labels' } },
    'composicion-fama': { labels:  { es: 'Nombres y valores',  en: 'Names and values' },
                          special: { es: 'Leyenda',            en: 'Legend' } },
    'explora-talento':  { special: { es: 'Leyenda',            en: 'Legend' } },
    'mapa-fama':        { special: { es: 'Leyenda',            en: 'Legend' } }
  };
  function knobLabel(key, lang) {
    const own = KNOB_LABEL[ae.chartId] && KNOB_LABEL[ae.chartId][key];
    if (own) return own[lang] || own.es;
    if (key === 'special' && SPECIAL_LABEL[ae.chartId]) {
      return SPECIAL_LABEL[ae.chartId][lang] || SPECIAL_LABEL[ae.chartId].es;
    }
    const m = SIZE_META[key];
    return m ? (m[lang] || m.es) : key;
  }

  // Qué perillas de tamaño obedece cada chart. Criterio (decidido con Daniel,
  // 2026-08-12): el panel NO muestra una perilla que el gráfico ignora — un
  // slider que no mueve nada es peor que no tener slider. La página puede
  // declararlo con data-editor-sizes="title,subtitle,chart,ticks,caption"
  // (así lo hace el N°5); este mapa es el default por chart, y si no hay ni
  // una cosa ni la otra se muestran las cuatro de siempre. Título, subtítulo,
  // nota y zoom solo aparecen donde la página los declara: el png-export de
  // lib/ todavía no los obedece.
  const SIZE_KEYS = {
    // N°1
    'bienestar-pib':      ['ticks', 'labels', 'axisTitle'],
    'homicidios-pib':     ['ticks', 'labels', 'axisTitle'],
    'homicidios-series':  ['ticks', 'special', 'axisTitle'],   // sin etiquetas de país
    // N°2
    'marimekko-gini':     ['ticks', 'labels', 'special', 'axisTitle'],
    'scatter-gini-pib':   ['ticks', 'labels', 'special', 'axisTitle'],
    'deciles':            ['ticks', 'special', 'axisTitle'],   // sin etiquetas de país
    'ranking':            ['ticks', 'labels', 'special', 'axisTitle'],
    // N°3
    'scatter-elo-pib':    ['ticks', 'labels', 'axisTitle'],
    // N°4
    'n5-ranking':         ['ticks', 'labels', 'special', 'axisTitle'],
    'n5-barrio-comp':     ['ticks', 'labels', 'special', 'axisTitle'],
    'n5-prioridad-comp':  ['ticks', 'labels', 'special', 'axisTitle'],
    'n5-migrantes-comp':  ['ticks', 'labels', 'axisTitle']
  };
  const ALL_SIZE_KEYS = ['ticks', 'labels', 'special', 'axisTitle'];
  function sizeKeysFor(chartId) {
    const block = document.querySelector('.chart-block[data-editor-sizes]');
    const declaradas = block ? (block.dataset.editorSizes || '').split(',').map(s => s.trim()).filter(Boolean) : [];
    if (declaradas.length) return declaradas.filter(k => SIZE_ORDER.indexOf(k) >= 0);
    return SIZE_KEYS[chartId] || ALL_SIZE_KEYS;
  }
  ...
  function defaultConfig(chartId, lang) {
    return {
      ...
      sizes: { ...DEFAULT_SIZES },
      // Qué sliders de tamaño tocó el lector. Los que no tocó siguen el preset
      // del formato activo (ver atlasEditorSize en lib/utils.js).
      sizesTouched: {},
      countries: [...(DEFAULT_COUNTRIES[chartId] || [])],
      format: pageDefaultFormat(),
      lang: lang || 'es'
    };
  }
  ...
  function loadConfig(chartId) {
    try {
      ...
      return {
        ...
        // Configs guardadas antes de que existiera sizesTouched: si algún
        // tamaño difiere del de fábrica, es que el lector lo eligió — se marca
        // como tocado para que su elección siga valiendo.
        sizesTouched: obj.sizesTouched || (() => {
          const t = {};
          Object.keys(DEFAULT_SIZES).forEach(k => {
            const v = (obj.sizes || {})[k];
            if (typeof v === 'number' && v !== DEFAULT_SIZES[k]) t[k] = true;
          });
          return t;
        })(),
        ...
        format: obj.format || pageDefaultFormat(),
        // El idioma NO se restaura de lo guardado: manda el de la PÁGINA.
        // `lang` dice qué juego de textos se está editando/dibujando AHORA, no
        // es una preferencia a recordar. Restaurándolo, una config guardada
        // alguna vez en castellano hacía que la MISMA página abierta en inglés
        // exportara el PNG con el título, el subtítulo y la nota en castellano
        // —y la firma en inglés, que sale del idioma de la página—: un PNG
        // mezclado. Lo leen 23 archivos vía `(ae.lang) || LANG`, así que el
        // arreglo tiene que estar acá, en el origen.
        lang: getLang()
      };
    } catch (_) {
      return null;
  ...
  function buildCountryUniverse() {
    ...
    try {
      // Hook genérico: el chart publica su propio universo de países en
      // window.__atlasCountryUniverse (array de iso3). Es la vía para los
      // charts que no están en la lista de abajo — sin esto, la sección
      // "Países etiquetados" no aparecía aunque el chart SÍ leyera la lista
      // (caso de los seis del N°4; lo reportó Daniel, 2026-08-11).
      if (Array.isArray(window.__atlasCountryUniverse) && window.__atlasCountryUniverse.length) {
        window.__atlasCountryUniverse.forEach(c => push(typeof c === 'string' ? c : (c && c.code)));
        return out;
      }
  ...
  // =========================================================
  // Sliders de tamaño: mientras el lector no movió uno, el panel muestra el
  // tamaño que el chart está DIBUJANDO (depende del formato: en newsletter los
  // ticks van a 20-24, no a los 11 de pantalla). Así el número del panel y lo
  // que se ve coinciden, y mover el slider agranda o achica DESDE ahí. Antes
  // el slider marcaba 11 con el gráfico en 20: moverlo achicaba y parecía roto.
  function syncSizeSliders() {
    if (!ae.panelEl || !ae.config) return;
    const cfg = ae.config;
    // Perillas que este chart obedece: las demás no se muestran.
    const visibles = sizeKeysFor(ae.chartId);
    SIZE_ORDER.forEach(k => {
      const row = ae.panelEl.querySelector(`[data-ae-size-row="${k}"]`);
      if (row) row.style.display = (visibles.indexOf(k) >= 0) ? '' : 'none';
    });
    visibles.forEach(k => {
      const slider = ae.panelEl.querySelector(`input[data-ae-size="${k}"]`);
      const valEl = ae.panelEl.querySelector(`[data-ae-size-val="${k}"]`);
      const resetEl = ae.panelEl.querySelector(`[data-ae-size-reset="${k}"]`);
      const tocado = !!(cfg.sizesTouched && cfg.sizesTouched[k]);
      const efectivo = (window.__atlasEffectiveSizes || {})[k];
      const v = (!tocado && typeof efectivo === 'number')
        ? efectivo
        : (cfg.sizes[k] != null ? cfg.sizes[k] : DEFAULT_SIZES[k]);
      if (slider) slider.value = v;
      if (valEl) valEl.textContent = fmtSize(k, v);
      // ↺ solo cuando la perilla está tocada: devuelve el tamaño al preset.
      if (resetEl) resetEl.style.visibility = tocado ? 'visible' : 'hidden';
    });
  }
  function fmtSize(k, v) {
    const n = Math.round(v * 10) / 10;
    return (SIZE_META[k] && SIZE_META[k].unit === '%') ? n + '%' : String(n);
  }
  ...
  function emit() {
    ...
    // Los charts redibujan al recibir el evento y recién ahí se sabe qué
    // tamaño quedó en uso: refrescamos los sliders en el tick siguiente.
    setTimeout(syncSizeSliders, 0);
    schedulePreview();
  }

  // =========================================================
  //  Vista previa del PNG (si la página la ofrece)
  // =========================================================
  // Título, subtítulo y nota existen solo dentro del PNG: sin una vista previa,
  // sus perillas no mostrarían nada en pantalla. La página expone
  // window.__atlasPngPreview() → Promise<canvas>, que compone EXACTAMENTE la
  // imagen que baja el botón. Se recompone (con demora) ante cualquier cambio
  // del editor o del gráfico.
  const pv = { timer: null, busy: false, dirty: false, observer: null, lastCanvas: null };
  function previewAvailable() {
    return typeof window.__atlasPngPreview === 'function';
  }
  function schedulePreview(delay) {
    if (!ae.panelEl || !previewAvailable()) return;
    clearTimeout(pv.timer);
    pv.timer = setTimeout(runPreview, delay == null ? 280 : delay);
  }
  async function runPreview() {
    if (!ae.panelEl || !previewAvailable()) return;
    // Con el panel cerrado no se compone nada (el export fuerza redibujos).
    if (!ae.panelEl.classList.contains('ae-open')) { pv.dirty = true; return; }
    if (pv.busy) { pv.dirty = true; return; }
    pv.busy = true; pv.dirty = false;
    const box = ae.panelEl.querySelector('[data-ae-preview]');
    if (box) box.classList.add('ae-preview-busy');
    try {
      const cv = await window.__atlasPngPreview();
      if (cv && box) {
        cv.classList.add('ae-preview-canvas');
        cv.removeAttribute('style');
        const dims = ae.panelEl.querySelector('[data-ae-preview-dims]');
        if (dims) dims.textContent = cv.width + ' × ' + cv.height;
        const lb = document.querySelector('.ae-lightbox');
        if (lb && lb.classList.contains('ae-open')) {
          // Ampliada: la imagen nueva reemplaza a la que se está mirando.
          lb.innerHTML = ''; lb.appendChild(cv);
        } else {
      ...
      console.error('[editor] vista previa del PNG falló:', err);
    } finally {
      // Los redibujos que hizo el export no son cambios del lector.
      if (pv.observer) pv.observer.takeRecords();
      pv.busy = false;
      if (box) box.classList.remove('ae-preview-busy');
      syncSizeSliders();
      if (pv.dirty) schedulePreview();
    }
  }
  // Cambios del gráfico hechos por el lector (filtros, selección, idioma).
  // Se miran los redibujos ENTEROS: los hijos directos del <svg> o del <tbody>
  // (cada draw los vacía y los rearma) y el texto de título, subtítulo y pie.
  // El hover toca atributos o grupos internos, no hijos directos, así que no
  // recompone el PNG a cada movimiento del mouse. Los redibujos que hace el
  // propio export se descartan (pv.busy + takeRecords).
  function watchChartForPreview() {
    if (pv.observer || !previewAvailable()) return;
    const block = findChartBlock();
    if (!block || typeof MutationObserver === 'undefined') return;
    pv.observer = new MutationObserver(() => {
      if (!pv.busy) schedulePreview(450);
    });
    block.querySelectorAll('svg[id^="chart"], tbody').forEach(el => {
      pv.observer.observe(el, { childList: true });
    });
    const textos = Array.from(block.querySelectorAll('.chart-title, .chart-subtitle'));
    const pie = document.querySelector('.footer .footer-row');
    if (pie) textos.push(pie);
    textos.forEach(el => {
      pv.observer.observe(el, { childList: true, subtree: true, characterData: true });
    });
  }
  // Clic en la miniatura: la misma imagen, grande, sobre la página.
  function openLightbox() {
    const box = ae.panelEl && ae.panelEl.querySelector('[data-ae-preview]');
    const cv = box && box.querySelector('canvas');
    if (!cv) return;
    let lb = document.querySelector('.ae-lightbox');
    if (!lb) {
    ...
    lb.innerHTML = ''; lb.appendChild(cv);
    lb.classList.add('ae-open');
  }
  function closeLightbox() {
    const lb = document.querySelector('.ae-lightbox');
    if (!lb || !lb.classList.contains('ae-open')) return;
    const cv = lb.querySelector('canvas');
    const box = ae.panelEl && ae.panelEl.querySelector('[data-ae-preview]');
    if (cv && box) { box.innerHTML = ''; box.appendChild(cv); }
    lb.classList.remove('ae-open');
    lb.innerHTML = '';
  }
  ...
  window.AtlasEditor = {
    ...
    triggerRender: emit,
    reloadFromStorage,
    // Valores de fábrica de los sliders de tamaño. Los charts los necesitan
    // para distinguir "el usuario movió el slider" de "está en el default"
    // (ver atlasEditorSize en lib/utils.js).
  ...
  function buildPanelDom() {
    ...
    panel.innerHTML = `
      ...
      <div class="ae-body">
        ...
        <div class="ae-section ae-preview-section" data-ae-section="preview" style="display:none">
          <h3 class="ae-section-title"><span data-ae-l="preview">Vista previa del PNG</span> <span class="ae-preview-dims" data-ae-preview-dims></span></h3>
          <div class="ae-preview" data-ae-preview role="button" tabindex="0"></div>
        </div>
        ...
        <div class="ae-section" data-ae-section="sizes">
          ...
          ${SIZE_ORDER.map(k => `
          <div class="ae-slider-row" data-ae-size-row="${k}">
            <label data-ae-size-label="${k}"></label>
            <input type="range" min="${SIZE_META[k].min}" max="${SIZE_META[k].max}" step="${SIZE_META[k].step}" data-ae-size="${k}">
            <span class="ae-slider-val" data-ae-size-val="${k}"></span>
            <button type="button" class="ae-size-reset" data-ae-size-reset="${k}" aria-label="Reset">↺</button>
          </div>`).join('')}
        </div>
  ...
  const PANEL_I18N = {
    es: {
      ...
      countries: 'Países etiquetados', sizes: 'Tamaños',
      ...
      preview: 'Vista previa del PNG', 'preview-zoom': 'Clic para ampliar',
      'size-reset': 'Volver al tamaño de fábrica'
    },
    en: {
      texts: 'Texts', title: 'Title', subtitle: 'Subtitle', caption: 'Caption',
      countries: 'Labeled countries', sizes: 'Sizes',
      ...
      preview: 'PNG preview', 'preview-zoom': 'Click to enlarge',
      'size-reset': 'Back to the default size'
    }
  };
  ...
  function applyPanelI18n() {
    ...
    // Nombre de cada perilla de tamaño: el del chart si lo redefine (el
    // "especial" siempre depende del chart), si no el genérico.
    ae.panelEl.querySelectorAll('[data-ae-size-label]').forEach(el => {
      el.textContent = knobLabel(el.getAttribute('data-ae-size-label'), lang);
    });
    ae.panelEl.querySelectorAll('[data-ae-size-reset]').forEach(el => {
      el.title = dict['size-reset'];
    });
    const pvBox = ae.panelEl.querySelector('[data-ae-preview]');
    if (pvBox) pvBox.title = dict['preview-zoom'];
  ...
  function syncUI() {
    ...
    showSection('preview', previewAvailable());
    // Charts con lienzo propio (las tablas del N°5, el mapa de alto dinámico)
    // no usan el formato del editor: la página lo declara y el selector se va.
    const blk = findChartBlock();
    showSection('format', !(blk && blk.getAttribute('data-editor-format') === 'off'));
    // Solo ofrecer los formatos que el número soporta (PNG_FORMATS del
    // utils cargado en la página — el N°1, p.ej., solo tiene 'square'), con
    // las medidas REALES de ese número: el N°2 y el N°5 definen newsletter
    // como 1000×1100, no el 1080×1080 que decía la etiqueta fija.
    const fmtOptSel = ae.panelEl.querySelector('select[data-ae-format]');
    if (fmtOptSel && typeof PNG_FORMATS !== 'undefined') {
      Array.from(fmtOptSel.options).forEach(o => {
        const f = PNG_FORMATS[o.value];
        if (!f) { o.remove(); return; }
        if (f.nominalW && f.nominalH) {
          o.textContent = o.textContent.replace(/\d+\s*×\s*\d+/, f.nominalW + '×' + f.nominalH);
        }
      });
    }
  ...
  function updatePngHint() {
    ...
    let txt = (FORMAT_HINT[lang] || FORMAT_HINT.es)[fmt] || '';
    // Medidas reales del número (ver syncUI) y nada si el chart no usa formatos.
    const f = (typeof PNG_FORMATS !== 'undefined') ? PNG_FORMATS[fmt] : null;
    if (f && f.nominalW && f.nominalH) txt = txt.replace(/\d+\s*×\s*\d+/, f.nominalW + ' × ' + f.nominalH);
    const blk = findChartBlock();
    if (blk && blk.getAttribute('data-editor-format') === 'off') txt = '';
  ...
  function wirePanelHandlers() {
    ...
    SIZE_ORDER.forEach(k => {
      ...
      const resetEl = panel.querySelector(`[data-ae-size-reset="${k}"]`);
      if (!slider) return;
      slider.addEventListener('input', () => {
        const v = parseFloat(slider.value);
        ae.config.sizes[k] = v;
        // A partir de acá este tamaño lo maneja el lector: pisa el preset del
        // formato (ver atlasEditorSize en lib/utils.js). Marca explícita para
        // no tener que adivinarlo comparando con el valor de fábrica.
        if (!ae.config.sizesTouched) ae.config.sizesTouched = {};
        ae.config.sizesTouched[k] = true;
        if (valEl) valEl.textContent = fmtSize(k, v);
        if (resetEl) resetEl.style.visibility = 'visible';
        saveConfig(ae.config);
        emit();
      });
      // ↺: la perilla vuelve a seguir el preset del formato (sin tocar las
      // demás ni los textos, que es lo que hace el Reset general).
      if (resetEl) resetEl.addEventListener('click', () => {
        if (ae.config.sizesTouched) delete ae.config.sizesTouched[k];
        ae.config.sizes[k] = DEFAULT_SIZES[k];
        saveConfig(ae.config);
        emit();
      });
    });

    // ----- Vista previa: clic amplía -----
    const pvBox = panel.querySelector('[data-ae-preview]');
    if (pvBox) {
      pvBox.addEventListener('click', openLightbox);
      pvBox.addEventListener('keydown', (ev) => {
        if (ev.key === 'Enter' || ev.key === ' ') { ev.preventDefault(); openLightbox(); }
      });
    }
    ...
    const SVG_CHART_NUMERIC = {
      // N°3 — los 12. Hasta 2026-08-11 solo estaban los 4 primeros: en los
      // otros ocho el botón quedaba oculto y el SVG era inalcanzable, aunque
      // svg-export.js estuviera cargado en la página. Los nombres de archivo
      // (los mismos que el PNG) están en lib/svg-export.js.
      'scatter-elo-pib':      '1',
  ...
  function mountEditor() {
    ...
    // LA SELECCIÓN DEL LINK MANDA (2026-08-11, lo encontró Daniel).
    // El editor guarda su propia lista de países (config.countries: la que
    // el usuario tildó, o la default del chart). Al montarse la volcaba
    // siempre, así que abrir un link compartido CON ?nl=1 mostraba los países
    // del editor y no los del link — el estado de la URL quedaba pisado.
    // Ahora: si la URL trae ?paises=, esa selección gana y el editor la
    // ADOPTA (así el panel muestra tildado lo que está dibujado, y editar
    // desde ahí parte de lo que el lector ve). Sin ?paises=, todo como antes.
    const paisesDelLink = (typeof atlasUrlParam === 'function') ? atlasUrlParam('paises') : null;
    if (chartId === 'deciles' && (typeof state !== 'undefined' && state[3])) {
      const yaAplicada = Array.isArray(state[3].selectedCountries) && state[3].selectedCountries.length;
      if (paisesDelLink && yaAplicada) {
        // el chart ya aplicó (y validó contra el dataset) la lista del link
        ae.config.countries = state[3].selectedCountries.slice();
      } else {
        state[3].selectedCountries = ae.config.countries.slice();
        if (typeof renderDecilesSelectedChips === 'function') {
          renderDecilesSelectedChips();
    ...
    } else if (paisesDelLink) {
      // Charts donde config.countries define qué se ETIQUETA (scatter del N°2,
      // mapa de clubes): que etiquete lo que pidió el link, no la lista vieja.
      const arr = paisesDelLink.split('~').filter(c => /^[A-Z]{3}$/.test(c));
      if (arr.length) ae.config.countries = arr;
    }
