...
// === Vista compartible ("Copiar link", Fase 5 — idea OWID) ===================
...
// (c) Botón "Copiar link" junto a los de descarga (uno por .footer). Se
//     inyecta solo si el chart lo pide (window.__atlasShareable = true — lo
//     setean los charts que mantienen su estado en la URL vía atlasSyncUrl,
//     y lib/grapher.js para sus páginas multi-vista). Copia la URL VIVA y
//     confirma en el propio botón ("¡Link copiado!", patrón OWID de feedback
//     inmediato). Fallback sin Clipboard API: textarea+execCommand; último
//     recurso: prompt con la URL para copiar a mano.
function atlasWireShareButton() {
  if (!window.__atlasShareable) return;
  const label = () => (typeof LANG !== 'undefined' && LANG === 'en') ? 'Copy link' : 'Copiar link';
  const done  = () => (typeof LANG !== 'undefined' && LANG === 'en') ? 'Link copied!' : '¡Link copiado!';
  const btns = [];
  document.querySelectorAll('.footer').forEach(f => {
    const anchor = Array.from(f.querySelectorAll('button.download')).pop();
    if (!anchor || f.querySelector('.atlas-share-btn')) return;
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'download atlas-share-btn';
  ...
  // Mantener el texto al togglear idioma (el botón no lleva data-i18n).
  if (btns.length && !atlasWireShareButton._langWired) {
    atlasWireShareButton._langWired = true;
    document.querySelectorAll('.lang-toggle button').forEach(b =>
      b.addEventListener('click', () => setTimeout(() => btns.forEach(x => { x.textContent = label(); }), 0)));
  }
}
window.addEventListener('load', () => setTimeout(atlasWireShareButton, 0));
