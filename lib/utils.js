...
function atlasEditorSize(aeSizes, clave, preset) {
  if (!aeSizes) return preset;
  const v = aeSizes[clave];
  if (typeof v !== 'number') return preset;
  const def = (window.AtlasEditor && window.AtlasEditor.defaultSizes)
    ? window.AtlasEditor.defaultSizes[clave] : undefined;
  return (typeof def === 'number' && v === def) ? preset : v;
}
