...
// Funciona en los dos sentidos: una página declara la dirección del OTRO idioma
// y se usa cuando se está leyendo en ese idioma. Los gráficos solo declaran la
// inglesa (la castellana es la propia página); las portadas de cada número, que
// son dos páginas reales con toggle, declaran cada una la de su contraparte.
const ATLAS_RUTA_REAL = (typeof location !== 'undefined') ? location.pathname : '';
function atlasSharePathname() {
  try {
    const lang = ((typeof LANG !== 'undefined' ? LANG : 'es') === 'en') ? 'en' : 'es';
    const alt = document.querySelector('link[rel="alternate"][hreflang="' + lang + '"]');
    if (alt) return new URL(alt.getAttribute('href'), location.href).pathname;
...
function atlasSyncDocTitle() {
  try {
    const el = document.querySelector('title[data-title-en], title[data-title-es]');
    if (!el) return;
    if (!el.dataset.tituloPropio) el.dataset.tituloPropio = document.title;   // el del idioma de la página, tal como vino del HTML
    const lang = ((typeof LANG !== 'undefined' ? LANG : 'es') === 'en') ? 'en' : 'es';
    const otroIdioma = (lang === 'en') ? el.dataset.titleEn : el.dataset.titleEs;
    const nuevo = otroIdioma || el.dataset.tituloPropio;
    if (nuevo && document.title !== nuevo) document.title = nuevo;
  } catch (_) {}
}
// Un link interno que DECLARA idioma sigue el idioma que se está leyendo. Las
// portadas en inglés linkean sus gráficos con `?lang=en` fijo: al pasar la
// portada a español, las tarjetas seguían abriendo el gráfico en inglés. Solo
// se tocan los links que ya traían el parámetro (marcados la primera vez, para
// no perderlos cuando se lo saca), y en español se saca — el castellano es el
// default y la URL limpia es la regla de la casa.
function atlasSyncLangLinks() {
  try {
    ...
    document.querySelectorAll('a[href]').forEach((a) => {
      ...
      const hash = corte >= 0 ? href.slice(corte) : '';
      const sinHash = corte >= 0 ? href.slice(0, corte) : href;
      const partes = sinHash.split('?');
      const qs = new URLSearchParams(partes[1] || '');
      if (!qs.has('lang') && a.dataset.langLink !== '1') return;
      a.dataset.langLink = '1';
      if (en) qs.set('lang', 'en'); else qs.delete('lang');
      const q = qs.toString();
      const nuevo = partes[0] + (q ? '?' + q : '') + hash;
...
if (typeof window !== 'undefined') {
  const alCambiarIdioma = () => { atlasSyncSharePath(); atlasSyncDocTitle(); atlasSyncLangLinks(); };
