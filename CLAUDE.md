...
- **Perillas de tamaño y vista previa (N°5, sep 2026)**: el bloque declara `data-editor-caps="sizes"` + `data-editor-sizes="title,subtitle,chart,ticks,axisTitle,labels,special,caption"` (solo las que obedece). Título/subtítulo/nota mueven el PNG; `chart` es el ZOOM DEL CONTENIDO (en SVG: viewBox 1/k, mismo recuadro, todo lo de adentro k veces más grande; en tablas: filas/retratos/columnas × k, entran menos filas; en barras de alto intrínseco el zoom agranda textos y las barras toman el alto que queda). Cada perilla lee `atlasEditorSize` (pisa el preset solo si el lector la movió; ↺ la devuelve). La página expone `window.__atlasPngPreview()` → canvas IDÉNTICO al que baja el botón, y el panel lo muestra en vivo. Charts de alto libre piden su hueco con `atlasPngBox(id, fmt)` para llenar lo que dejan los textos (nunca franjas muertas). Lienzo propio (tablas, mapa de alto dinámico) → `data-editor-format="off"` y el selector de formato se va. El editor arranca en `__atlasDefaultPngFormat` (el mismo PNG que baja el lector). Hoy lo obedece el N°5; el `lib/png-export.js` de los N°1–4 todavía no.
